...
float pos = 0;
float anglerror = 0;
float angleerraccum = 0;
float posaccum = 0;
float ELai = 0;
float ERai = 0;
float anglerrorR = 0;
float anglerrorL = 0;
float angleerraccumR =0;
float angleerraccumL = 0;
...
void updatePWMs(float totalDistanceLeft, float totalDistanceRight, 
float vL, float vR, float angleRad, float angleRadAccum, 
float  errorLAccum, float errorRAccum, float delta_t, 
float pos, float anglerrorR,float anglerrorL, float angleerraccumR,
float angleerraccumL) {
   ...
   */
  float Kp = -17.9;  //-108;//-7; // 13.8 6
  float Ki = -34.5; //-89;//-52; // 21 41
  float Jp = 100; //7.7;//136; // 190 150
  float Ji = 1000; //94;//600; // 885 5
  /*
   * 
   */
  float pc = -0;//0.02;
  ...
  pos = (vL + vR)/2 * delta_t;
  posaccum += pos;
  anglerrorR = Td-angleRad+(totalDistanceLeft)*pc;//pc*posaccum + angleRad;
  anglerrorL = Td-angleRad+(totalDistanceRight)*pc;
  angleerraccumR += anglerrorL * delta_t;
  angleerraccumL += anglerrorL * delta_t;
  float vdR = Kp*anglerrorR + Ki*angleerraccumR;
  float vdL = Kp*anglerrorL + Ki*angleerraccumL;
  ELai += (vdL - vL) *delta_t;
  ERai += (vdR - vR)*delta_t;
  leftMotorPWM = (Jp*(vdL - vL) + Ji*ELai)  ;
  rightMotorPWM = (Jp*(vdR - vR) + Ji*ERai) ;
  
  Serial.print("LeftMotor PWM: ");Serial.print(leftMotorPWM);
  Serial.print(" -- RightMotor PWM: ");Serial.println(rightMotorPWM);
  Serial.print(vR);Serial.print("right"); Serial.println(vL);

//  float lPWM= Jp*(vd - vL) + Ji*errorLAccum;
//  float rPWM = Jp*(vd - vR) + Ji*errorRAccum;
//  Serial.print("LeftMotor PWM: ");
//  Serial.print(lPWM);
//  Serial.print(" -- RightMotor PWM: ");
//  Serial.println(rPWM);
//  if (leftMotorPWM >300){
//      leftMotorPWM = 300;
//      rightMotorPWM = 300;}
...
/* Uncomment this and comment the above if doing wireless
  ...
  if(cur_time - prev_time > UPDATE_TIME_MS && start_flag)
  {
    ...
    updatePWMs(totalDistanceLeft, totalDistanceRight, vL, vR, 
    angle_rad, angle_rad_accum, error_left_accum, error_right_accum, 
    delta_t, pos, anglerrorR, anglerrorL, angleerraccumR,angleerraccumL);
